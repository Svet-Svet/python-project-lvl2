...
def stylish(graph, _deep=0):
    ...
    for node in graph:
        ...
        if isinstance(node[2], list):
            if status == ADDED:
                keys = f'\n{count_spaces}+ {is_bool(node[1])}: '
                result += keys + stylish(is_bool(node[2]), _deep + 4)
            elif status == REMOVED:
                keys = f'\n{count_spaces}- {is_bool(node[1])}: '
                result += keys + stylish(is_bool(node[2]), _deep + 4)
                ...
                old = f'\n{count_spaces}- {is_bool(node[1])}: '
                new = f'\n{count_spaces}+ {is_bool(node[1])}: {is_bool(node[3])}'
                result += old + stylish(is_bool(node[2]), _deep + 4)
                result += new

        elif status == CHANGED and isinstance(node[3], list):
                old = f'\n{count_spaces}- {is_bool(node[1])}: {is_bool(node[2])}'
                new = f'\n{count_spaces}+ {is_bool(node[1])}: '\
                      + stylish(is_bool(node[3]), _deep + 4)
                result += old
                result += new
        elif status == ADDED:
            result += f'\n{count_spaces}+ {is_bool(node[1])}: {is_bool(node[2])}'
